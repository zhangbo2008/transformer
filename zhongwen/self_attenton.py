#这个代码是用self_attention模型来做汉子和拼音的匹配,算是一个简单的翻译模型.

'''
以后全部用docker
把代码,数据,环境都弄一起,然后发布到dockerhub,这样最方便后续调用了.
就是显卡gpu,问题麻烦.
'''
...
inputs = []
labels = []
for i in tqdm(range(len(data))):#数据用/t和空格分隔.
...
pny2id = get_vocab(inputs) #结果是一个数组
...
def get_batch(input_data, label_data, batch_size):
    ...
    for k in range(batch_num):
        ...
        input_batch = np.array([line + [0] * (max_len - len(line)) for line in input_batch])#根据maxlen 补长度,补的就是前面说的'pad'
        label_batch = np.array([line + [0] * (max_len - len(line)) for line in label_batch])
        yield input_batch, label_batch #yield返回的是一个生成器,用next不停的取,每一个for循环的结果
#只有当next触发时候for循环就跑一次,否则for循环停住.
...
def multihead_attention(emb,
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        # Split and concat  #把axis=2的数据都切8份.
        Q_ = tf.concat(tf.split(Q, num_heads, axis=2), axis=0)  # (h*N, T_q, C/h)
        K_ = tf.concat(tf.split(K, num_heads, axis=2), axis=0)  # (h*N, T_k, C/h)
        V_ = tf.concat(tf.split(V, num_heads, axis=2), axis=0)  # (h*N, T_k, C/h)

        # Multiplication     ,这里面论文就是做矩阵乘法.
        outputs = tf.matmul(Q_, tf.transpose(K_, [0, 2, 1]))  # (h*N, T_q, T_k)

        # Scale
        outputs = outputs / (K_.get_shape().as_list()[-1] ** 0.5)

        # Key Masking    key_masks 结果是1,0矩阵,
        key_masks = tf.sign(tf.abs(tf.reduce_sum(emb, axis=-1)))  # (N, T_k)
        key_masks = tf.tile(key_masks, [num_heads, 1])  # (h*N, T_k)
        key_masks = tf.tile(tf.expand_dims(key_masks, 1), [1, tf.shape(queries)[1], 1])  # (h*N, T_q, T_k)

        paddings = tf.ones_like(outputs) * (-2 ** 32 + 1) #如果key_masks结果是0,就输出负无穷
...
with tf.Session() as sess:
    ...
    sess.run(tf.global_variables_initializer())
    if os.path.exists('./logs/model.meta'): #如果存在就继续进行finetune
        print('找到了之前的模型,所以直接进行finetune')
...
with tf.Session() as sess:
    saver.restore(sess, './logs/model')
    while True:
        line = input('输入测试拼音句子: ') #输入一段拼音用空格分隔,然后返回中文.也看做一种翻译.
        if line == 'exit': break
        line = line.strip('\n').split(' ')
        tmp=[]
        for pny in line:
            try:
                tmp.append(pny2id.index(pny))
            except:
                tmp.append(0)
        x=np.array(tmp)
